...
nodes/
  dev/
    ...
    apps/               Piri, Ingot
scripts/
  host/                 run on the node: provision, deploy, reconcile, keygen
...
4. Run `provision-platform.sh` on the node: initialise OpenBao, generate keys, start the platform.
5. Onboard against central, then run `provision-apps.sh`.
6. Enable `filone-reconcile.timer`.

Two of those need work that lands in other repositories first, and the runbook says which.
