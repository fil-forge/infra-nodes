...
variable "ucantool_version" {
  description = "Release tag of the ucantool binary to install, e.g. v0.3.0. fil-forge/ucantool publishes no release binaries yet; while this is empty the node boots without ucantool and keygen.sh refuses to run."
  type        = string
  default     = ""
}

variable "ucantool_sha256" {
  description = "SHA-256 of the linux arm64 ucantool binary named by ucantool_version. Required whenever that version is set: an unverified binary generates the node's identity keys."
  type        = string
  default     = ""
}

variable "foundry_version" {
  description = "Release tag of the Foundry archive the `cast` binary is extracted from. Only cast is installed; forge, anvil and chisel are discarded."
  type        = string
  default     = "v1.7.1"
}

variable "foundry_sha256" {
  description = "SHA-256 of foundry_<version>_linux_arm64.tar.gz, published as a .sha256 asset alongside it on the release."
  type        = string
  default     = "c8fe8fa09ae3aba2c81b510c6f9da3a9d468029b9580e690b245b3f0aea687ae"
}
