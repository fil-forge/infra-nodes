...
if [ -f /etc/filone/ucantool-missing ]; then
  die "$(cat /etc/filone/ucantool-missing)"
fi
command -v ucantool >/dev/null || die "ucantool is not on PATH"
command -v cast >/dev/null || die "cast is not on PATH"

# --- Identities -------------------------------------------------------------

# ucantool prints the PEM on stdout and the DID on stderr, and has no way to
# read the DID back out of an existing PEM. So the DID is stored beside the key
# at the moment it is generated, and every later run reads it rather than
# deriving it. A `ucantool identity did <file>` would let this be derived on
# demand and is worth adding upstream.
generate_identity() {
  local name="$1"
  ...
  if bao_has "$name" identity_pem; then
    bao_has "$name" did ||
      die "$name has a key in OpenBao but no DID, and the DID cannot be derived from the key.
       Recover it from the node's registration at central and write it to $FILONE_BAO_MOUNT/$name#did."
