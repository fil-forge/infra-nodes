...
if [ ! -r "$FILONE_SEAL_TOKEN_FILE" ]; then
  ...
  echo "[1/6] Seal token already present"
fi

# --- 2. OpenBao -------------------------------------------------------------

echo "[2/6] Starting OpenBao"
write_openbao_env
...
if [ ! -r "$FILONE_BAO_TOKEN_FILE" ]; then
  echo "[3/6] Creating the deploy token"
...
POLICY
  ...
  echo "[3/6] Deploy token already present"
fi

# --- 4. Keys and passwords --------------------------------------------------

echo "[4/6] Generating keys and passwords"
[ -x "$SCRIPT_DIR/keygen.sh" ] ||
  die "scripts/host/keygen.sh is missing or not executable in $FILONE_CHECKOUT"
"$SCRIPT_DIR/keygen.sh"

# --- 5. Operator-supplied secrets -------------------------------------------

echo "[5/6] Operator-supplied secrets"
prompt_secret() {
...
echo "[6/6] Starting Postgres, Caddy and Alloy"
