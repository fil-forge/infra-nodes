...
Three things a node needs are built elsewhere. The first blocks everything; the other two block
parts of bring-up.
...
**Release binaries for ucantool**, in fil-forge/ucantool: static linux arm64 builds on a GitHub
release. Until they exist, `ucantool_version` and `ucantool_sha256` in
`terraform/envs/dev/terraform.tfvars` stay empty, the box boots without ucantool, and `keygen.sh`
refuses to run and says so. Everything up to key generation works.
...
It then asks for the root token back, to create the deploy token and the KV mount; generates the
node's keys; asks for the chain.love and Grafana Cloud tokens; and starts Postgres, Caddy and Alloy.
